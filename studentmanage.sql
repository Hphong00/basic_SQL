...
select student.name, student.gender, student.faculty_id, 
case 
...
select count(id) totalstudent from student ;
...
select student.faculty_id,faculty.name, count(student.id)
from student inner join faculty on student.faculty_id = faculty.id
group by student.faculty_id,faculty.name;


-- 5. Tính tổng số sinh viên của từng môn học

select  e.subject_id,sj.name, count(e.student_id)
from (exam_management e join subject sj on e.subject_id = sj.id) join student s on e.student_id = s.id
group by e.subject_id,sj.name;
...
select e.student_id, count( distinct  subject_id)
...
select faculty_id, sum(scholarship)
from student
group by faculty_id;
...
select faculty_id,f.name, max(nvl(scholarship, 0))
from student s join faculty f on s.faculty_id = f.id
group by faculty_id,f.name;

-- 9. Cho biết tổng số sinh viên nam và tổng số sinh viên nữ của mỗi khoa 
--hỏi
select faculty_id,count(case when gender = 'Nam' then 1 else 0 end )  totalNam,
count(case when gender = 'Nữ' then 1 else 0 end )  totalNU
...
select e.student_id,e.subject_id, count(distinct e.number_of_exam_taking)
from exam_management e  join student s on e.student_id = s.id
...
---- 13. Cho biết những sinh viên nam có điểm trung bình lần 1 trên 7.0 
 ...
 select e.student_id, avg(mark) avg_mark
 from student s join exam_management e on s.id = e.student_id
...
-- 14. Cho biết danh sách các sinh viên rớt ít nhất 2 môn ở lần thi 1 (rớt môn là điểm thi của môn không quá 4 điểm) 
 --hỏi
 select e.student_id,count(e.subject_id) 
 from student s join exam_management e on s.id = e.student_id
...
select s.faculty_id, count(DISTINCT s.id) 
from  student s
where s.gender = 'Nam'
...
-- 16. Cho biết những khoa có 2 sinh viên đạt học bổng từ 200000 đến 300000 
 select s.faculty_id, count(s.faculty_id)
 from student s join faculty f on s.faculty_id = f.id
 where scholarship between 100000 and 150000
 group by s.faculty_id 
 having count(s.faculty_id)=2;
...
from student s
where   to_number(to_char(sysdate,'YYYY')) - to_number(to_char(s.birthday,'YYYY'))>20 ;

---- 3. Sinh viên sinh vào mùa xuân năm 1990
select *
from student s
where to_char(s.birthday, 'MM') in ('01', '02', '03') 
...
--
select s.name, f.name faculty_name,s.gender
from student s
join faculty f on s.faculty_id = f.id
where (f.name = 'Anh - Văn' 
    or f.name = 'Tin học') and s.gender='Nam';
...
--where e.number_of_exam_taking = 1 and  e.mark = 
--(select max(e.mark) from exam_management e);
--
---- 4. Cho biết sinh viên khoa anh văn có tuổi lớn nhất.
select s.name, f.name,s.faculty_id , max(s.birthday)
from student s
join faculty f on s.faculty_id = f.id
where f.name ='Anh - Văn'
group by s.name, f.name,s.faculty_id, s.birthday 
having max(s.birthday)>= 
...
select faculty.name, count(student.id) student_number
from faculty, student    
where faculty.id = student.faculty_id and student.gender='Nữ'
group by faculty.name 
...
select e.student_id,s.name, max(e.mark)
from (student s join exam_management e on s.id = e.student_id) join subject sj on e.subject_id = sj.id
...
--select * from exam_management;
--
--select DISTINCT e.student_id,s.name, e.number_of_exam_taking
--from (student s join exam_management e on s.id = e.student_id) 
--where e.number_of_exam_taking !=1 and e.number_of_exam_taking =2
